    ...
    layout_sidebar(
      sidebar = sidebar(
        ...
        accordion(
          ...
          accordion_panel(
            "Partial PPO",
            actionButton("btn_train_par", "Train Partial PPO", class = "btn-primary btn-sm w-100")
          ),
          accordion_panel(
            "Multinomial Reg.",
        ...
        selectInput("cmp_model", "Model",
                    choices = c("Random Forest", "XGBoost", "kNN",
                                "Ordinal Reg.", "Partial PPO", "Multinomial Reg.")),
        selectInput("cmp_partition", "Partition",
                    choices = c("A (pollutants)", "B (all features)"),
                    selected = "B (all features)"),
        hr(),
        p(em("Partial PPO is only available for Partition A."), style = "font-size:0.82em; color:#888;")
      ),
      layout_columns(
        col_widths = c(12),
        card(
      ...
      layout_columns(
        col_widths = c(6, 6),
        card(card_header("Confusion Matrix"),                 plotOutput("plot_cm",      height = "360px")),
        card(card_header("RF vs XGBoost Feature Importance"), plotOutput("plot_imp",     height = "360px"))
      ),
      card(
        card_header("Poor-class Precision-Recall Curve"),
        plotOutput("plot_pr", height = "360px")
      )
    )
  ),
    ...
    layout_sidebar(
      sidebar = sidebar(
        ...
        actionButton("btn_train_sc3", "Run Feature Selection",
                     class = "btn-success w-100", icon = icon("play")),
        p(em("Requires Multinomial Reg. (B) to be trained first."),
          style = "font-size:0.82em; color:#888; margin-top:6px;")
      ),
      layout_columns(
        col_widths = c(12),
        card(
    ...
    layout_sidebar(
      sidebar = sidebar(
        ...
        actionButton("btn_train_sc4", "Train Trees",
                     class = "btn-success w-100", icon = icon("play")),
        p(em("Requires Random Forest (B) to be trained first."),
          style = "font-size:0.82em; color:#888; margin-top:6px;"),
        hr(),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$btn_train_par, {
    withProgress(message = "Training Partial PPO…", value = 0, {
  ...
  observeEvent(input$btn_train_all, {
    withProgress(message = "Training all models…", value = 0, {
      ...
      setProgress(0.74, detail = "Partial PPO…")
      partial_A <- fit_partial(train_data, PARTITION_A)
      rv_models(modifyList(rv_models(), list(partial_A = partial_A)))
  ...
  observeEvent(input$btn_train_sc4, {
    ...
    withProgress(message = "Training decision trees (Scenario 4)…", value = 0, {
      ...
      preds_rf_B_for_tree <- predict_rf(m$rf_B, test_data, PARTITION_B)$preds
      ...
      depth_sweep <- tibble(maxdepth = 2:12) %>%
        mutate(
          ...
          agreement_rf = map_dbl(preds_tree, ~ mean(.x == preds_rf_B_for_tree)),
  ...
  summaries <- reactive({
    ...
    a_only_specs <- list(
      list(key = "p_par_A", method = "Partial PPO", type = "Parametric")
    )
    ...
    if (!is.null(m$partial_A)) {
      pr_models_list[["Partial PPO A"]] <- PRROC::pr.curve(
  ...
  sc3_data <- reactive({
    ...
    preds_multinom_B <- p$p_mul_B$preds
    actual_sc3       <- as.integer(as.character(test_data$AQI_p24))

    sc3_metrics <- bind_rows(
      compute_metrics(preds_multinom_B, actual_sc3) %>% mutate(Method = "Multinomial B (baseline)"),
    ...
    sc3_recall <- bind_rows(
      compute_class_recall(preds_multinom_B, actual_sc3) %>% mutate(Method = "Multinomial B"),
  ...
  output$tbl_sc3_features <- renderTable({
    ...
    sc3_data()$feature_selection_table %>%
      mutate(
        Lasso_1se = ifelse(Lasso_1se, "✓", "✗"),
        ElNet_1se = ifelse(ElNet_1se, "✓", "✗"),
        StepAIC   = ifelse(StepAIC, "✓", "✗")
      )
  }, striped = TRUE, hover = TRUE)
  ...
  output$plot_depth_sweep <- renderPlot({
    ...
    req(!is.null(s4))
    s4$depth_sweep %>%
      pivot_longer(c(agreement_rf, accuracy),
      ...
      data = s4$depth_sweep %>%
          ...
          summarise(n_leaves = first(n_leaves),
                    y = max(agreement_rf, accuracy) * 100 + 1.5),
  ...
  output$tbl_tree_metrics <- renderTable({
    s4 <- rv_sc4()
    p  <- all_preds()
    req(!is.null(s4), !is.null(p$p_rf_B))
