...
The core assumption of ordinal regression is proportional odds, the coefficients that desribe relationships between categories are same. 
...
It is possible to evaulate model A, as it did converge in the end, making it usable for predictions. The coefficent can be interpreted similarly as explained in ordered regression and multinomial regression (below) sections.
```{r}
...
The partial proportional odds model is very close to the full multinomial regression as only two features are fulfilling the PO assumption. It also solves the issue with impossible probabilities, as multinomial regression treats every class independently.
```{r}
...
We can interpret the exponentiated coefficients as odds ratios against the base class, which is AQI level 1 (Good). All of the pollutant values have higher coefficients with higher category, meaning that increasing value of pollutants have increased odds of being in higher levels of AQI, as it should. For example with increasing pollutant PM2.5, the odds are 1.04 times higher for AQI 2 than AQI 1. For third level it is 1.07 and for fourth 1.09. Values lower than 1 means that it is has less odds, rain usually does not mean higher level of AQI. We can visualize this by plotting proportion of rain for each category. Very high odds ratios, for example with snowfall, means that data has only few occurrences of it. This can make the predictions for days with snow very innacurate, as the large coefficient is much larger than pollutant ones.
...
#### Eval multinominal
The macro F1 and poor class recall is significantlly better compared to the partial PO model. When it comes to the ordinal regression, the F1 scores are simillar but the recall for poor class is much higher.
```{r}
