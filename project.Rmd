...
summary_table %>%
  ...
  ggplot(aes(x = Method, y = `Macro F1`, fill = Partition)) +
...
The heatmap reveals a split between non-parametric and parametric.  Non-parametric models (RF, XGBoost) maintain relatively balanced recall thanks to the class weighting approach. 

Parametric models have a worse poor recall than non-parametric models. They use less agressive class weighting to optimize their Macro F1 scores, but that comes at a cost.

@TODO: rozdiel parametric vs non parametric
...
We use Precision-Recall (PR) curves to evaluate the ability of models to detect the Poor class.
...
RF and XGBoost are the strongest predictors for the Poor class, substantially above the random baseline. Parametric models are noticeably behind the non-parametric models, except for KNN which is essentially at the random baseline, and not useful for detecting the Poor class at all.
